...
class DecorationController extends Controller
{
    ...
    public function create()
    {
        $user = User::where('role', 'vendor')
            ...
            ->get();
        return view('admin.vendor.decoration.product', compact('user'));
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('decoration.index')->with('success', 'Product has been successfully created');
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $user = User::find($id);
        if (!$user) {
            return redirect()->route('decoration.index')->with('error', 'User not found');
        }

        $product = Product::where('user_id', $id)->first();
        if (!$product) {
            return redirect()->route('decoration.index')->with('error', 'Product not found');
        }
